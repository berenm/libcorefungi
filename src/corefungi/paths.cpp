...
namespace corefungi {
  ...
  namespace {
    ...
    static bfs::path const getpath(std::string const& variable, bfs::path const& fallback, std::string const& append="") {
    ...
    static bfs::path const default_global_config_dir = (cfg::expand("~") / ".config").string();
    static bfs::path const default_global_cache_dir  = (cfg::expand("~") / ".cache").string();
    static bfs::path const default_global_data_dir   = (cfg::expand("~") / ".local" / "share").string();
    ...
    static bfs::path const get_prefix(bfs::path const& program) {
      ...
      return bin_dir.has_parent_path() ? bin_dir.parent_path() : cfg::get_default_prefix(program);
    }

  }

  std::string const global_config_dir(std::string const& program) { return (global_config_dirs.at(cfg::platform) / bfs::canonical(program).stem()).string(); }
  std::string const global_cache_dir(std::string const& program) { return (global_cache_dirs.at(cfg::platform) / bfs::canonical(program).stem()).string(); }
  std::string const global_data_dir(std::string const& program) { return (global_data_dirs.at(cfg::platform) / bfs::canonical(program).stem()).string(); }
  ...
  std::string const system_cache_dir(std::string const& program) { return (bfs::canonical(program).root_path() / "var" / "cache" / bfs::canonical(program).stem()).string(); }
  std::string const system_data_dir(std::string const& program) { return (cfg::get_prefix(program) / "share" / bfs::canonical(program).stem()).string(); }
