...
namespace corefungi {
  ...
  namespace {
    ...
    static bfs::path const expand(std::string const& path) {
      if (path.empty()) return bfs::path();

      if (path[0] != '~') return bfs::path(path);

      std::string modified = path;
      if ((path.size() > 1) && (path[1] != '/')) modified.insert(1, "/../");
    ...
    static cfg::string_map const global_cache_dirs
      = {{"linux", cfg::getpath("XDG_CACHE_HOME", default_global_cache_dir)},
         {"Win32",
          cfg::getpath("LOCALAPPDATA", default_global_cache_dir, "Cache")},
