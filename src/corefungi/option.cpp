...
namespace corefungi {
  ...
  std::string option::name() const {
    if (std::get< 0 >(this->implicit_value)
        && (std::get< 2 >(this->implicit_value) != "")) {
      std::string msg = "[=arg(=" + std::get< 2 >(this->implicit_value) + ")]";
      if (std::get< 0 >(this->default_value)
          && (std::get< 2 >(this->default_value) != ""))
        msg += " (=" + std::get< 2 >(this->default_value) + ")";

      return msg;
    } else if (std::get< 0 >(this->default_value)
               && (std::get< 2 >(this->default_value) != "")) {
  ...
  unsigned option::max_tokens() const {
    return std::get< 0 >(this->implicit_value) ? 0
                                               : (this->multitoken ? -1 : 1);
  }
  ...
  void option::xparse(boost::any&                       value_store,
    ...
    if (!this->multitoken) {
      ...
      if (value_store.empty()) value_store = boost::any(cfg::spore_list{});
  ...
  bool option::apply_default(boost::any& value_store) const {
    if (!std::get< 0 >(this->default_value)) return false;

    if (!this->multitoken) {
      value_store = std::get< 1 >(this->default_value);
    } else {
      value_store = cfg::spore_list{std::get< 1 >(this->default_value)};
    }
  ...
  void option::notify(boost::any const& value_store) const {
    if (!this->multitoken) {
      cfg::put(cfg::command, this->option_name,
               boost::any_cast< cfg::spore >(value_store));
    } else {
      auto const& values = boost::any_cast< cfg::spore_list >(value_store);
      cfg::grow(cfg::command,
                this->option_name + ".#" + std::to_string(values.size() - 1));
