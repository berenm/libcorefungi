...
namespace corefungi {
  ...
  namespace {
    ...
    template < typename OutT > struct transform_result {
      typedef std::vector< OutT > type;
    };

    template < typename KeyT, typename ValueT >
    struct transform_result< std::pair< KeyT, ValueT > > {
      typedef std::unordered_map< typename std::remove_cv< KeyT >::type,
                                  ValueT >
        type;
    };
    ...
    template < typename InT, typename OpT,
               typename ValT = typename InT::value_type >
    static auto transform(InT& i, OpT&& lambda) ->
      typename transform_result< decltype(lambda(*std::begin(i))) >::type {
  ...
  cfg::ref_dict items(cfg::node& node) {
    return cfg::transform(boost::get< cfg::dict >(node), [](cfg::pair& v) {
  ...
  cfg::spore_list keys(cfg::node& node) {
    return cfg::transform(boost::get< cfg::dict >(node),
                          [](cfg::pair& v) { return cfg::spore{v.first}; });
  }

  cfg::ref_list values(cfg::node& node) {
    return cfg::transform(boost::get< cfg::dict >(node),
                          [](cfg::pair& v) { return cfg::node_ref{v.second}; });
  }
}
