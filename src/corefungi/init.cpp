...
namespace corefungi {
  ...
  namespace {
    ...
    static corefungi::sprout const o
      = {"General options",
         {{"program.help", "produces this help message",
           cfg::long_name = "help", cfg::short_name = "h", cfg::bool_switch},
          {"program.config", "custom configuration file",
           cfg::long_name = "config", cfg::short_name = "c"}}};
  }
  ...
  void init(std::string const& program, cfg::arguments const& arguments) {
    ...
    if (bfs::exists(local_config)) yaml::read(local_config, cfg::local);
