  ...
  namespace detail {
    ...
    template < typename T > struct reference {
     public:
      ...
      template < typename U >
      using if_derive_T =
        typename std::enable_if< std::is_base_of< T, U >::value, void >::type;

      /** constructors and assignment operators for T object */
      template < typename U, typename = if_derive_T< U > >
      explicit reference(U const& u) : pointer(&u) {}
      template < typename U, typename = if_derive_T< U > >
      explicit reference(U& u) : pointer(&u) {}

      /** dereference the T object */
      template < typename U = T, typename = if_derive_T< U > >
      operator U const&() const {
        return *static_cast< U const* >(this->pointer);
      }
      template < typename U = T, typename = if_derive_T< U > > operator U&() {
        return *static_cast< U* >(this->pointer);
      }
      ...
      template < typename U = T, typename = if_derive_T< U > > U* operator&() {
        return static_cast< U* >(this->pointer);
      }
      ...
      template < typename U > reference& operator=(U&& u) {
        this->pointer->operator=(std::forward< U >(u));
        return *this;
      }
    ...
    static inline std::basic_ostream< Cr, Tr >& operator<<(
      std::basic_ostream< Cr, Tr >& s, detail::reference< T > const& r) {
