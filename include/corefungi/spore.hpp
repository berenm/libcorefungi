...
namespace corefungi {
  ...
  struct spore : std::string {
    ...
    template < typename T >
    using if_sporable =
      typename std::enable_if< cfg::is_spore_convertible< T >::value,
                               void >::type;

    template < typename T, typename = if_sporable< T > >
    explicit spore(T const& t)
      : std::string(cfg::lexical_cast< std::string >(t)) {}
  ...
  static inline std::basic_ostream< Cr, Tr >& operator<<(
    std::basic_ostream< Cr, Tr >& s, cfg::spore const& sp) {
